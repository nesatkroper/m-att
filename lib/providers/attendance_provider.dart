...
class AttendanceProvider with ChangeNotifier {
  ...
  Future<void> processQrScan(String qrData, String employeeId) async {
    ...
    try {
      ...
      // Process QR scan with mock API
      final result = await _apiService.processAttendanceScan(qrData, employeeId);
  ...
  Map<String, int> getMonthlyAttendanceStats() {
    ...
    final monthRecords = _recentRecords.where((record) => 
    ...
    final approvedLeaves = _leaveRequests.where((leave) => 
      leave.status == 'approved' && (
        (leave.startDate.year == now.year && leave.startDate.month == now.month) ||
        (leave.endDate.year == now.year && leave.endDate.month == now.month)
      )
    ).toList();
